...
int e1000_transmit(struct mbuf *m)
{
  ...
  // tx_mbufs is set to 0 initially so this is a good way to know if there is something
  if (tx_mbufs[ring_offset])
  {
    mbuffree(tx_mbufs[ring_offset]);
  }

  tx_mbufs[ring_offset] = m;
